...
//                var smoothPatients: (Any)? = nil;
                ...
                db.collection("doctors").document(email ?? "test@test.com").collection("smooth").getDocuments() { (querySnapshot, err) in
                    if let err = err {
...
//                    //            "rightdata": rightDummyData,
//                    ])
//                let docRef2 = db.collection("doctors")
//
//                var references: [Any] = []
